...
Instale as dependências do Python: Crie um arquivo requirements.txt com o conteúdo abaixo ou instale manualmente:
...
Instale os binários do navegador: O Playwright precisa baixar o navegador para funcionar.

Bash

playwright install
▶️ Como Utilizar
Inicie o Aplicativo: No terminal, dentro da pasta do projeto:
...
Analisando os Resultados: O app divide os dados encontrados em 3 abas:
...
Exportação: Cada tabela encontrada possui um botão "Baixar CSV" logo abaixo dela.

🧠 Como Funciona (Arquitetura Técnica)
O aplicativo segue um fluxo linear de dados:

Scheduler: Recebe as URLs e cria uma fila de execução.

Downloader (Async Playwright):
...
Interface (Streamlit): Renderiza os DataFrames e gerencia o loop de eventos assíncronos (com correção ProactorEventLoop para Windows).
